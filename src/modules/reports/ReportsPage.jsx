    ...
    <div className="animate-in">
      <div className="card" style={{ marginBottom:16 }}>
        <div style={{ display:'flex', gap:12, alignItems:'flex-end', flexWrap:'wrap' }}>
          ...
          <button className="btn btn-outline"  onClick={doPrint}  disabled={!filteredWings.length}>🖨️ طباعة</button>
          <button className="btn btn-green"    onClick={doExcel}  disabled={!filteredWings.length}>📊 Excel</button>
          <button
            className={`btn btn-sm ${showPrev ? 'btn-blue' : 'btn-outline'}`}
            onClick={() => setShowPrev(v => !v)}
            disabled={!filteredWings.length}
      ...
      {!loading && filteredWings.length === 0 && (
        <div className="empty-state"><div className="es-icon">📭</div><div className="es-title">لا توجد بيانات لهذا التاريخ والجولة المحددة</div></div>
      )}
        ...
        <>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:16 }}>
            {[
              ...
              {label:'أجنحة مُقيَّمة',    value:filteredWings.length,color:'#057a55', bg:'#e3f9ee'},
            ].map((s,i)=>(
